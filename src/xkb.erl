...
-spec init(Format, Keymap) -> state()
      when Format :: xkb_v1,
           Keymap :: string() | binary().
...
-spec update_modifiers(ModsDepressed, ModsLatched, ModsLocked, Group, State0) -> State1
      when State0 :: state(),
           State1 :: state(),
           ModsDepressed :: integer(),
           ModsLatched :: integer(),
           ModsLocked :: integer(),
           Group :: integer().
update_modifiers(ModsDepressed, ModsLatched, ModsLocked, Group, State) ->
    Mods = (ModsDepressed bor ModsLatched bor ModsLocked) band ?MODS_REAL_MASK,
    State#state{modifiers=Mods, group=Group}.


-spec key_pressed(Key, State0) -> {[Symbol], State1}
      when State0 :: state(),
           State1 :: state(),
           Key :: integer(),
           Symbol :: {char, char()} | {key, atom()}.
key_pressed(Key, #state{keymap=Keymap,modifiers=Mods,group=Group}=State) ->
    {keysym_get(Key, Keymap, Mods, Group), State}.


-spec key_released(Key, State0) -> {[Symbol], State1}
      when State0 :: state(),
           State1 :: state(),
           Key :: integer(),
           Symbol :: {char, char()} | {key, atom()}.
key_released(Key, #state{keymap=Keymap,modifiers=Mods,group=Group}=State) ->
    {keysym_get(Key, Keymap, Mods, Group), State}.
...
parse_keymap(String) ->
    {ok, Tokens, _} = xkb1_lexer:string(String),
    {ok, Keymap} = xkb1_parser:parse(Tokens),
    Keymap.
